...
def parse_alert(text):
    ...
    if m:
        return {
            ...
            "order_type":  "pending",          # FIX: no hardcodear "market"
            "direction":   m.group(1).upper(),
            "symbol":      m.group(2).upper(),
            "timeframe":   m.group(3),
            "entry_price": float(m.group(4)),  # "Entrada ahora" — solo referencia
            "bos_level":   float(m.group(5)),  # nivel limite real
            "sl":          float(m.group(6)),
            "tp1":         float(m.group(7)),
            "tp2":         float(m.group(8)),
            "tp3":         float(m.group(9)),
    ...
    if m2:
        return {
            ...
            "timeframe":   m2.group(3),
            "score":       int(m2.group(4)) if m2.group(4) else 0,
            "entry_price": float(m2.group(5)),
            "sl":          float(m2.group(9)),
            "tp1":         float(m2.group(6)),
            "tp2":         float(m2.group(7)),
            "tp3":         float(m2.group(8)),
        }
    ...
    if "callback_query" in data:
        ...
        if parts[0] == "entry":
            ...
            if sig_id not in _pending:
                answer_cb(cb_id, "⚠️ Señal expirada")
                return "ok", 200
            # FIX: actualizar order_type Y entry_price correctamente
            ...
            if not entry or entry["sig_id"] != sig_id:
                answer_cb(cb_id, "⚠️ Confirmación expirada")
                return "ok", 200
        ...
        if key in _pending:
            ...
            try:
                ...
                return "ok", 200
            if sig_id not in _pending:
                send_message(chat_id, "⚠️ Señal expirada.")
                return "ok", 200
...
def _do_order(chat_id, sig_id, usdt):
    if sig_id in _executed:
        send_message(chat_id, "⚠️ Esta orden ya fue ejecutada.")
        return
...
def status():
    return {"status": "running", "pending": len(_pending)}, 200


def register_webhook():
    if not RENDER_URL or not TELEGRAM_TOKEN:
        print("[TG] Sin RENDER_URL/TOKEN, omitiendo setWebhook")
        return
    try:
        r = requests.post(
